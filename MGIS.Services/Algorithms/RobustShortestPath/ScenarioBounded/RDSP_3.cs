...
{
    public class RDSP_3 : IBoundedRDSP
    {
        public string UniqueId => "RDSP_3";
        ...
        public ShortestPathProblemOutput Run(int[][][] costs, int startPointIndex, int endPointIndex, Graph graph)
        {
            ...
            // dp[t,v,pr_0,...,pr_S]
            int[,,,,] dp = new int[costs[0].Length - 1, costs[0].Length, Pr[0] + 1, Pr[1] + 1, Pr[2]];
            ...
            for (int s1 = -Zr[0]; s1 < Pr[0] - Zr[0]; s1++)
            {
                ...
                {
                    for (int s3 = -Zr[2]; s3 < Pr[2] - Zr[2]; s3++)
                    {
                        for (int v = 0; v < costs[0].Length; v++)
                        {
                            dp[0, v, s1 + Zr[0], s2 + Zr[1], s3 + Zr[2]] = Math.Min(Math.Max(Math.Max(s1 + costs[0][v][endPointIndex], s2 + costs[1][v][endPointIndex]), s3 + costs[2][v][endPointIndex]), PrMax);
                        }
                    }
                }
            }
                ...
                for (int v = 0; v < costs[0].Length; v++)
                {
                    for (int s1 = -Zr[0]; s1 < Pr[0] - Zr[0]; s1++)
                    {
                        for (int s2 = -Zr[1]; s2 < Pr[1] - Zr[1]; s2++)
                        {
                            for (int s3 = -Zr[2]; s3 < Pr[2] - Zr[2]; s3++)
                            {
                                dp[t, v, s1 + Zr[0], s2 + Zr[1], s3 + Zr[2]] = PrMax;
                                for (int v2 = 0; v2 < costs[0].Length; v2++)
                                {
                                    if (
                                        (s1 + costs[0][v][v2] < Pr[0] - Zr[0] && s2 + costs[1][v][v2] < Pr[1] - Zr[1] && s3 + costs[2][v][v2] < Pr[2] - Zr[2])
                                        && dp[t - 1, v2, s1 + Zr[0] + costs[0][v][v2], s2 + Zr[1] + costs[1][v][v2], s3 + Zr[2] + costs[2][v][v2]] < dp[t, v, s1 + Zr[0], s2 + Zr[1], s3 + Zr[2]]
                                        )
                                    {
                                        dp[t, v, s1 + Zr[0], s2 + Zr[1], s3 + Zr[2]] = dp[t - 1, v2, s1 + Zr[0] + costs[0][v][v2], s2 + Zr[1] + costs[1][v][v2], s3 + Zr[2] + costs[2][v][v2]];
                                    }
                                }
                            }
                        }
            ...
            while ((nextpoint != endPointIndex) && (t_r > 0))
            {
                for (int v = 0; v < costs[0].Length; v++)
                {
                    if (s1_r + costs[0][nextpoint][v] < Pr[0] + 1 && s2_r + costs[1][nextpoint][v] < Pr[1] + 1 && s3_r + costs[2][nextpoint][v] < Pr[2] && dp[t_r, nextpoint, s1_r, s2_r, s3_r] == dp[t_r - 1, v, s1_r + costs[0][nextpoint][v], s2_r + costs[1][nextpoint][v], s3_r + costs[2][nextpoint][v]])
                    {
