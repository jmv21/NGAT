...
{
    public class ARSP_2 : IBoundedARSP
    {
        public string UniqueId => "ARSP_2";
        ...
        public ShortestPathProblemOutput Run(int [][][] costs, int startPointIndex, int endPointIndex, Graph graph)
        {
            ...
            for (int s1 = 0; s1 < Pr[0]; s1++)
            {
                for (int s2 = 0; s2 < Pr[1]; s2++)
                {
                    ...
                    {
                        dp[0,v,s1,s2] = Math.Min(Math.Max(s1 + costs[0][v][endPointIndex],s2 + costs[1][v][endPointIndex]), PrMax);
                    }
                }
            }
                ...
                for (int v = 0; v < costs[0].Length; v++)
                {
                    for (int s1 = 0; s1 < Pr[0]; s1++)
                    {
                        for (int s2 = 0; s2 < Pr[1]; s2++)
                        {
                            ...
                            {
                                if (
                                    (s1 + costs[0][v][v2] < Pr[0] && s2 + costs[1][v][v2] < Pr[1])
                                    && dp[t-1, v2, s1+costs[0][v][v2], s2+costs[1][v][v2]] < dp[t, v, s1,s2]
                                    )
                                {
                                    dp[t, v, s1, s2] = dp[t - 1, v2, s1 + costs[0][v][v2], s2 + costs[1][v][v2]];
                                }
                            }
                        }
                    }
