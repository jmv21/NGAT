...
{
    public class BruteForceRDSP : IShortestPathProblemAlgorithm
    {
        public string UniqueId => "Brute Force RDSP";
        ...
        {
            List<(List<int>, List<double>)> paths = Utils.CalculatePaths(costs, sp.Id-1, ep.Id-1);
            double bestDeviation = double.MaxValue;
            int indexOf = 0;
            double [] bestDistance = new double[costs.Length];
            ...
            for (int i = 0; i < bestDistance.Length; i++)
            {
                ...
                dynamic result = algorithm.Run(scenario,scenario.NodesId[sp.Id], scenario.NodesId[ep.Id]);
                bestDistance[i] = result.Distance;
            }
            for (int i = 0; i < paths.Count; i++)
            {
                ...
                {
                    if (paths[i].Item2[j] - bestDistance[j]> worst)
                        worst = paths[i].Item2[j]- bestDistance[j];
                }
                if (worst < bestDeviation)
                {
                    bestDeviation = worst;
                    indexOf = i;
                }
            ...
            ShortestPathProblemOutput outPut = new ShortestPathProblemOutput(bestDeviation, nodesId, paths[indexOf].Item1.Select(n => new PointLatLong(graph.NodesId[n+1].Latitude, graph.NodesId[n+1].Longitude)), sp, ep);
            return outPut;
